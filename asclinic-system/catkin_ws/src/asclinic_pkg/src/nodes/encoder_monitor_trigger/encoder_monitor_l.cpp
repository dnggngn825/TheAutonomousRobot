...
Pololu_SMC_G2_Encoder encoder = Pololu_SMC_G2_Encoder(AMR_CW_DIRECTION_MOTOR_L);
...
{
	// Initialise the node
	ros::init(argc, argv, amr_node::ENCODER_MONITOR_L);
	ros::NodeHandle nd;


    // publisher_4_motor_control = nd.advertise<MotorAngularVelocity>(amr_topic::ENCODER_DATA_4_MOTOR_CONTROL_L, 10, false);    
    // publisher_4_odometry_control = nd.advertise<MotorRotationAngle>(amr_topic::ENCODER_DATA_4_ODOMETRY_CONTROL_L, 10, false);
    
	publisher_4_motor_control = nd.advertise<std_msgs::UInt16>(amr_topic::ENCODER_DATA_4_MOTOR_CONTROL_L, 10, false);
    publisher_4_odometry_control = nd.advertise<std_msgs::UInt16>(amr_topic::ENCODER_DATA_4_ODOMETRY_CONTROL_L, 10, false);
...
void subscriberCallbackForMotorControl(const std_msgs::UInt32& msg)
{
	std_msgs::UInt16 encoder_counter;
...
void subscriberCallbackForOdometryControl(const std_msgs::UInt32& msg)
{
	// ROS_INFO_STREAM("[ENCODER MONITOR L] Encoder counter in Odom (L): "<<encoder_counter_4_odometry_control);
	std_msgs::UInt16 encoder_counter;
