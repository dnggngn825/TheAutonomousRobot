...
{
	// Initialise the node
	ros::init(argc, argv, amr_node::ENC_MONITOR_R_CHANNEL_A);
	ros::NodeHandle nd;

    publisher_enc_state = nd.advertise<amr_msgs::EncLineState>(amr_topic::ENC_R_CHANNEL_A_STATE, 10, false);
	...
	msg.data = gpiod_ctxless_get_value(AMR_GPIO_CHIP_NAME, AMR_ENCODER_PIN_CHANNEL_A_MOTOR_R, false, "foobar");
	publisher_enc_state.publish(msg);

	// Loop
	while (ros::ok())
	{
        int event_type = encoder.get_channel_a_event_type(gpiod_line_event);
        if (event_type >0)
		{
			msg.data = event_type;
			publisher_enc_state.publish(msg);
