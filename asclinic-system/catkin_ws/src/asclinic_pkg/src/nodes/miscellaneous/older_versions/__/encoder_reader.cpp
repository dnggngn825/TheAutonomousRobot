...
const float TIMER_4_MOTOR_CONTROL_PUBLISHING = 0.02;
const float TIMER_4_ODOMETRY_CONTROL_PUBLISHING = 0.1;
...
ros::Timer timer_4_motor_control_publishing;
...
void timerCallbackForMotorControlPublishing(const ros::TimerEvent&);
void timerCallbackForOdometryControlPublishing(const ros::TimerEvent&);
void subscriberCallback(const MotorRotationAngle& msg);
...
void subscriberCallback(const MotorRotationAngle& msg)
{
	ROS_INFO_STREAM("[ENCODER READER] Message receieved with data (motor left : right)" << msg.rotation_angle_motor_l <<
					";" << msg.rotation_angle_motor_r);
}

void timerCallbackForMotorControlPublishing(const ros::TimerEvent&)
{
	// // Declare a static integer for publishing a different number each time
	// static uint callback_counter = 0;
	// callback_counter++;
    // ROS_INFO("Node callback: [%d] times.", callback_counter);

	float rotation_angle_motor_l = calculate_rotation_angle(encoder_counter_4_motor_control_motor_l, TIMER_4_MOTOR_CONTROL_PUBLISHING);
	float rotation_angle_motor_r = calculate_rotation_angle(encoder_counter_4_motor_control_motor_r, TIMER_4_MOTOR_CONTROL_PUBLISHING);
	...
	// Publish the message
	publisher_4_motor_control.publish(motor_rotation_angle);

    reset_encoder_counter_4_motor_control();

	// START THE TIMER AGAIN
	// > Stop any previous instance that might still be running
	timer_4_motor_control_publishing.stop();
	// > Set the period again (second argument is reset)
	timer_4_motor_control_publishing.setPeriod( ros::Duration(TIMER_4_MOTOR_CONTROL_PUBLISHING), true);
	// > Start the timer again
	timer_4_motor_control_publishing.start();
}
...
int main(int argc, char* argv[])
{
    ...
    // Publisher and timer for MOTOR CONTROL 
    publisher_4_motor_control = nodeHandle.advertise<MotorRotationAngle>("encoder_data_motor_ctl_topic", 10, false);
    timer_4_motor_control_publishing = nodeHandle.createTimer(ros::Duration(TIMER_4_MOTOR_CONTROL_PUBLISHING), timerCallbackForMotorControlPublishing, true);
...
/* bool are_encoders_ready(ros::NodeHandle nodeHandle)
{
    bool are_encoders_ready = false;
    
    bool is_ready_channel_A_motor_L = is_pin_ready(nodeHandle, AMR_ENCODER_PIN_CHANNEL_A_MOTOR_L);
    bool is_ready_channel_B_motor_L = is_pin_ready(nodeHandle, AMR_ENCODER_PIN_CHANNEL_B_MOTOR_L);
    bool is_ready_channel_A_motor_R = is_pin_ready(nodeHandle, AMR_ENCODER_PIN_CHANNEL_A_MOTOR_R);
    bool is_ready_channel_B_motor_R = is_pin_ready(nodeHandle, AMR_ENCODER_PIN_CHANNEL_B_MOTOR_R);

    if (is_ready_channel_A_motor_L && is_ready_channel_B_motor_L && 
        is_ready_channel_A_motor_R && is_ready_channel_B_motor_R)
    {
        ROS_INFO("[GPIO EVENT TRIG. ENC] All encoder pins are ready!");
        are_encoders_ready = true;
    }
    else 
    {
        ROS_INFO("[GPIO EVENT TRIG. ENC] NOT all encoder pins are ready!");
        are_encoders_ready = false;
    }
        
    return are_encoders_ready;
} */
...
bool are_encoder_pins_ready(ros::NodeHandle nodeHandle, int encoder_pins[])
{
	int encoder_pins_array_size = sizeof(encoder_pins)/sizeof(encoder_pins[0]);
