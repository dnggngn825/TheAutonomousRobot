...
//
// $ rostopic pub -once /motor_pwm_duty_cycle MotorAngularVelocityPWM '{pwm_duty_cycle_motor_l: 10, pwm_duty_cycle_motor_r:10}'


/* 
This Node:	Reads PWM signal sent from: 1. "amr_controller" in minimal mode/ 2. "motor_controller" in full mode.
			Generate PWM duty cycle to motor.
It acts like the physical component.

Next update: input signal in rad/s, replacing pwm.
 */
...
int main(int argc, char* argv[])
{
	...
	ros::Subscriber motor_pwm_duty_cycle_subscriber = nodeHandle.subscribe("motor_pwm_duty_cycle", 1, subscriberCallbackForMotorPwmDutyCycle);
	...
	for (int smc_pointer_index=0; smc_pointer_index<2; smc_pointer_index++)
	{
		...
		set_current_limit(pololu_smc_pointer, true);
		usleep(1000); // Short sleep

		// Send the max speed limit
		set_max_speed_limit(pololu_smc_pointer, true);
		usleep(1000); // Short sleep

		// Set the max acceleration limit
		set_max_accel_limit(pololu_smc_pointer, true);
		usleep(1000); // Short sleep
		
		// Set the max deceleration limit
		set_max_decel_limit(pololu_smc_pointer, true);
		usleep(1000); // Short sleep
	...
	while (ros::ok())
	{
		...
		// Spin once so that this node can service any callbacks that this node has queued.
		ros::spin();
