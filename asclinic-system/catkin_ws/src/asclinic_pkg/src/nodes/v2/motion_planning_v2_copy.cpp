...
int main(int argc, char* argv[])
{
    ...
    while (ros::ok())
    {
        ...
        if ((rb_state == amr_state::FSM::ST_IDLE) && (transitPath.noPoints))
        {
            ...
            ROS_INFO_STREAM("[TRANSIT] POSE TARGET index: " << transitPath.noPoints-2);
            index = 0;
        }
        ...
        else if ((rb_state == amr_state::FSM::DR_DELIVERY) && (index % PAUSE_INDEX == 0))
        {
        ...
        // PAUSE ON DELIVERY STATE
        else if ((rb_state == amr_state::FSM::DR_RETURN) && (index % PAUSE_INDEX == 0))
        {
