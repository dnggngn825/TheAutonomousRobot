...
int main(int argc, char* argv[])
{
    ...
    ros::Subscriber subscriber_4_motor_l = nd.subscribe(amr_topic::ENCODER_DATA_4_MOTOR_CONTROL_L, 10, subscriberCallbackForMotorLeft);
    ros::Subscriber subscriber_4_motor_r = nd.subscribe(amr_topic::ENCODER_DATA_4_MOTOR_CONTROL_R, 10, subscriberCallbackForMotorRight);
...
void publishAngularVelocityForMotorControl()
{
    if (received_data_encoder_l && received_data_encoder_r)
    {
        ...
        // if (counter == 10)
        // {
        //     ROS_INFO_STREAM("[ENCODER_MONITOR_4_MOTOR_CONTROL] Angular vel (L/R): " << angular_velocity_l << ";" << angular_velocity_r);
        //     counter = 0;
        // }
    }
}
