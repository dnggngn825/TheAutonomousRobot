...
float rescale_angular_velocity(float angular_vel, float error, bool is_rotating);
...
void subscriberCallback4PoseEstimated(const AmrPose& msg)
{
    ...
    if (in_movement)
    {
        ...
        if (rotation_after_pause)
        {
            error_location.current = 0;
            float deltaY = pose_target.pose_y - pose_current.pose_y;
            float deltaX = pose_target.pose_x - pose_current.pose_x;
            error_heading.current = withInPi(atan2(deltaY,deltaX) - (pose_current.pose_phi));
        }
        else if (current_movement_type)
        {
            // Rotation
            error_location.current = 0;
        ...
        else
        {
            ...
            float deltaY = pose_target.pose_y - pose_current.pose_y;
            float deltaX = pose_target.pose_x - pose_current.pose_x;
            ROS_INFO_STREAM("[TRAJ. CTL] POSE TARGET: "<<pose_target.pose_x<< ", "<< pose_target.pose_y<< std::endl);
            error_heading.current = withInPi(atan2(deltaY,deltaX) - (pose_current.pose_phi));
        ...
        if ((angular_velocity_l.current <0.01f) || (angular_velocity_r.current <0.01f))
        {
...
float PController4HeadingLinear(discreteParam error_heading)
{
    PIDController headingController = PIDController(1.0,0,0);
    return headingController.discreteP(error_heading);
}
...
float rescale_angular_velocity(float angular_vel, float error, bool is_rotating)
{
    ...
    {
        if ((is_rotating) && (abs(error) >= angle_thr))
            dtheta = get_sign(angular_vel) * (pwm_lower_limit * angular_vel_coeff);
        else 
            dtheta = 0.0f;
    }
    else if (abs(angular_vel) > pwm_upper_limit)
